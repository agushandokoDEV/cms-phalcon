...
<div class="panel panel-default">
  ...
  <div class="panel-body">
    ...
    <div class="media">
        ...
        <div class="media-body"> 
            <a href="/index/detail/{{val_post.post_slug}}"><h4 class="media-heading heighlight">{{val_post.post_title|stripslashes}}</h4></a> 
            <span class="label label-success">{{val_post.category}}</span>
            <br />
            <div class="heighlight"><?php $this->myhelp->char_limit(preg_replace("#<(.*)/(.*)>#iUs", "", $val_post->post_content),400) ?></div>
        </div> 
    </div>
    <hr />
    {% endfor %}
  ...
  <div class="panel-footer" style="padding: 0;">
    <nav class="text-center nav-pills-blue">
      <ul class="pagination pagination-blue" style="margin: 10px 0;">
        <li>
          <a href="/index/search?keyword={{keyword_search}}" aria-label="Previous">
            <span aria-hidden="true"><i class="fa fa-backward"></i></span>
          </a>
        </li>
        <li><a href="/index/search?keyword={{keyword_search}}&page={{content.list_posting.before}}">Prev</a></li>
        <li><a href="/index/search?keyword={{keyword_search}}&page={{content.list_posting.next}}">Next</a></li>
        <li>
          <a href="/index/search?keyword={{keyword_search}}&page={{content.list_posting.last}}" aria-label="Next">
  ...
  <script>
    $(document).ready(function(){
        $('.heighlight').highlight('{{keyword_search}}', {
            color: 'black',
            background: 'yellow',
            ignoreCase: true,
            wholeWord: false,
