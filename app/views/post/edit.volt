...
{% block pagetop %}
<a href="/post/index/{{row_post.post_type}}" title="Back" class="btn btn-default"><span class="glyphicon glyphicon-arrow-left"></span></a>
{% endblock %}
{% block content %}
<style>
.img-list-media{
...
{{ flash.output() }}
<form id="form-edit-post" method="POST" action="/post/edit/{{row_post.post_type}}/{{data.row.post_id}}">
<div class="row">
    <div class="col-md-8">
        ...
        <div class="form-group">
            ...
            <div class="input-group input-group">
                <input id="post-title" type="text" class="form-control" placeholder="Title" id="post-title" value="{{data.row.post_title|stripslashes}}"  name="title" autofocus required>
            ...
            <span class="text-sm" id="txt-slug">Slug : <i>{{data.row.post_slug}}</i></span>
            <input type="hidden" name="post_id" value="{{data.row.post_id}}"/>
            <input type="hidden" name="post_slug" id="post-slug" value="{{data.row.post_slug}}"/>
            <input type="hidden" id="media-post-cover" name="media_cover" value="{{data.row.post_cover}}"/>
        ...
        <div class="form-group">
            ...
            <div class="input-group input-group">
                <input id="pac-input" class="controls-maps form-control" type="text" placeholder="Masukan nama lokasi" value="{{data.row.event_location_name}}">
                    <span class="input-group-btn">
        ...
        <div class="form-group">
            ...
            <input type="hidden" id="location-name" name="location-name" value="{{data.row.event_location_name}}"/>
            <input type="hidden" id="location-addres" name="location-addres" value="{{data.row.event_location_addres}}"/>
            <input type="hidden" id="location-lat" name="location-lat" value="{{data.row.event_location_lat}}"/> 
            <input type="hidden" id="location-long" name="location-long" value="{{data.row.event_location_long}}"/>     
        </div>
        <div class="form-group">
            ...
            <textarea id="wysiwyg" class="form-control" rows="5" name="content"><?php echo htmlentities($data->row->post_content) ?></textarea>
        </div>
    ...
    <div class="col-md-4">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control dp2" name="schedule" id="post-schedule" value="{{data.row.publish_schedule}}"/>
        </div>
        <div class="form-group">
            <div class="checkbox">
                <label>
                  <input id="input-reminder" name="input_reminder" type="checkbox" value="{{data.row.post_reminder}}"> Reminder
                </label>
            </div>
        </div>
        <div class="form-group hidden" id="box-input-reminder">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <input name="event_start" id="event_start" class="controls-maps form-control dp2" type="text" placeholder="Start" value="{{data.row.event_date_start}}">
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group">
                        <input name="event_end" id="event_end" class="controls-maps form-control dp2" type="text" placeholder="End" value="{{data.row.event_date_end}}">
                    </div>
                </div>
            </div>
        </div>
        <div class="form-group">
            ...
            <div id="box-post-cover">{{data.post_cover}}</div>
        </div>
        <hr />
        <button class="btn btn-primary" id="btn-submit" style="width: 100%;">Publish</button>
        <?php $this->partial("layouts/html/box/end"); ?>
...
var media_cover_selected='{{data.row.post_cover}}';
...
var map,
    ...
    infowindowContent,
    marker;
var lat_default=parseFloat('{{config.api.google.maps.default.lat}}');
var lng_default=parseFloat('{{config.api.google.maps.default.lng}}');
var zoom=parseInt('{{config.api.google.maps.default.zoom}}');
var reminder='{{data.row.post_reminder}}';
...
$(document).ready(function(){
    ...
    $('#fil_lang').val('{{data.row.post_language}}');
    $('#post-stts').val('{{data.row.post_status}}');
    if($('#post-stts').val() == 'draft'){
    ...
    media_attach='{{data.list_attach}}'.split(',');
    if(media_attach.length > 0){
        //edited_media_attach=true; 
    ...
    $('#list-post-tags').select2().select2('val',{{data.data_post_tags}});
    ...
    //input-reminder
    if(reminder == 'Y'){
        $('input#input-reminder').attr('checked','checked');
        $('#box-input-reminder').removeClass('hidden');
        $('#event_start')
        .val('{{data.row.event_date_start}}')
        .attr('required','required');
        $('#event_end')
        .val('{{data.row.event_date_end}}')
        .attr('required','required');
    }
...
function insertTags(dt){
    $.post('/post/index/{{row_post.post_type}}/insert_post_tags',{dt_tags:dt,post_type:'{{row_post.post_type}}'},function(r){
...
function open_media(){
    ...
    /*
    console.log(edited_media_attach);
    open_selected_tab();
    console.log(tes);
    if(!edited_media_attach){
        //media_attach='{{data.list_attach}}'.split(',');
        if(media_attach.length > 0){
            check_media_attach=true;
            for(i=0;i<media_attach.length; i++){
                console.log(media_attach[i]);
...
function media_selected(dt_media_id){
    $('#box-btn-set-cover').html('');
    edited_media_attach=true;
    if(check_media_attach== true && tab_active=='attach'){
        if(media_attach.indexOf(dt_media_id) >= 0){
            ...
            del_item_media_attach(dt_media_id);
        }else{
...
function create_post_slug(){
    ...
    if(post_title.val() !=''){
        ...
        $.get('/post/index/{{row_post.post_type}}/get_post_slug',{post_title:post_title.val(),postid:'{{data.row.post_id}}'},function(r){
            var rs=jQuery.parseJSON(r);
            if(rs.success){
...
function checked_attach(){
    if(media_attach.length > 0){
        ...
        $.get('/post/index/{{row_post.post_type}}/list_media_attach',{media_attach:media_attach},function(r){
            var rs=jQuery.parseJSON(r);
            for(i=0; i<media_attach.length; i++){
                if(!$('div#list-media-attch').hasClass("checked-attach-selected-"+media_attach[i])){
                    $('#list-media-attch').append('<div class="checked-attach-selected-'+media_attach[i]+'" id="checked-attach-selected-'+media_attach[i]+'"><input type="hidden" name="list_media_attch[]" value="'+media_attach[i]+'"/></div>');
                }
            }
...
function initAutocomplete() {
    var row_lat='{{data.row.event_location_lat}}';
    var row_lng='{{data.row.event_location_long}}';
    
    if(row_lat != '' && row_lng != ''){
        lat_default=parseFloat(row_lat);
        lng_default=parseFloat(row_lng);
        zoom=17;
    }
