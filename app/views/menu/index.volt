...
{% block content %}
{{ flash.output() }}
<a href="/menu/add" class="btn btn-default"><i class="fa fa-plus"></i> Add new data</a>
    ...
    "columnDefs": [
        {
            ...
            "render": function(data, type, row, meta){
               var str='';
               str += '<a href="/menu/edit/'+row.menu_id+'" title="Edit Menu" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>';
               str += ' <a href="#" onclick="del_row('+"'"+row.menu_id+"'"+')" title="Delete Menu" class="btn btn-xs btn-default"><i class="fa fa-trash-o"></i></a>';
               return str;
            }
        },
        {
...
function del_row(pk)
{
    ...
    if(conf)
    {
        $.post('/menu/delete',{pk:pk},function(r){
            var rs=jQuery.parseJSON(r);
            if(rs.success)
            {
                alert(rs.message);
